...
class CircleSlider extends React.Component {
    ...
    render() {
        ...
        const endCoord2 = this.polarToCartesian(84);
        const radiusX = dR;
        const radiusY = dR;
        ...
        return (<react_native_1.View style={{ flex: this.props.flex ? this.props.flex : 1 }}>
                <react_native_svg_1.Svg onLayout={this.measureLocation} ref="circleslider" width={width} height={width} flex={1}>
                    ...
                    <react_native_svg_1.Path stroke={this.props.bgInactive} 
                        strokeWidth={this.props.sliderWidth} fill="none" strokeLinecap="round" strokeLinejoin="round" 
                        d={`M${startCoord.x} ${startCoord.y} A ${radiusX} ${radiusY} ${xAxisRotation} ${largeArc2} ${sweepFlag} ${endCoord2.x} ${endCoord2.y}`}/>
        <react_native_svg_1.Path stroke={"url(#gradient1)"} strokeWidth={this.props.sliderWidth} fill="none" strokeLinecap="round" strokeLinejoin="round" d={`M${startCoord.x} ${startCoord.y} A ${radiusX} ${radiusY} ${xAxisRotation} ${largeArc} ${sweepFlag} ${endCoord.x} ${endCoord.y}`} onPressIn={(e) => {
            ...
            <react_native_1.View overflow="visible" position="absolute" top={this.state.measuredBox[2].y} left={this.state.measuredBox[2].x} width={this.state.measuredBox[0].x - this.state.measuredBox[2].x} height={this.state.measuredBox[1].y - this.state.measuredBox[2].y}>
